...
class LiveUpdateSettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    private val liveUpdateTimeoutPrefKey = longPreferencesKey("LiveUpdateTimeout")
    ...
    internal val liveUpdateTimeoutFlow: Flow<Long?> =
        dataStore.data
            ...
            .map { settings -> settings[liveUpdateTimeoutPrefKey] }
    ...
    suspend fun setLiveUpdateTimeout(timeoutMs: Long) {
        dataStore.edit { it[liveUpdateTimeoutPrefKey] = timeoutMs }
        Log.i(TAG, "Live update timeout set to $timeoutMs")
    }
}
